...
def create_trainer(preprocess_fn, network_factory, read_from_file, image_shape,
    ...
    tf.compat.v1.summary.image("images", image_var)
...
def _create_softmax_loss(feature_var, logit_var, label_var):
    ...
    #label_var = tf.reshape(label_var, [1, -1])
#    cross_entropy_var = tf.losses.sparse_softmax_cross_entropy(
#        logit_var, tf.cast(label_var, tf.int64))
    cross_entropy_var = tf.compat.v1.losses.sparse_softmax_cross_entropy(
        tf.cast(label_var, tf.int64), logit_var)
    tf.compat.v1.summary.scalar("cross_entropy_loss", cross_entropy_var)

#    accuracy_var = slim.metrics.accuracy(
#        tf.cast(tf.argmax(logit_var, 1), tf.int64), label_var)
    accuracy_var = slim.metrics.accuracy(
        label_var, tf.cast(tf.argmax(logit_var, 1), tf.int64))
    tf.compat.v1.summary.scalar("classification accuracy", accuracy_var)
...
def _create_magnet_loss(feature_var, logit_var, label_var, monitor_mode=False):
    ...
    tf.compat.v1.summary.scalar("magnet_loss", magnet_loss)
    if not monitor_mode:
...
def _create_triplet_loss(feature_var, logit_var, label_var, monitor_mode=False):
    ...
    tf.compat.v1.summary.scalar("triplet_loss", triplet_loss)
    if not monitor_mode:
